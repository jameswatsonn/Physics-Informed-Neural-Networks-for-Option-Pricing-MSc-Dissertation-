...
class Heston(Equation):
    ...
    def __init__(self, eqn_config):
        super(Heston, self).__init__(eqn_config)
        params = eqn_config.model_params
        
        # Model parameters from config file
        self.initial_stock = params['initial_stock']
        self.initial_vol = params['initial_vol']
        self.r = params['r']   # Risk-free rate
        self.kappa = params['kappa']  # Mean reversion speed
        self.theta = params['theta']  # Long-term variance
        self.epsilon = params['epsilon']  # Volatility-of-volatility
        self.rho = params['rho']   # Correlation
        self.strike = params['strike']   # Strike price
        
        # Initial state [stock_price, variance]
        self.x_init = np.array([self.initial_stock, self.initial_vol])

    # Generate sample paths for Heston model 
    def sample(self, num_sample):
        # Generate independent Brownian increments (with dim=2 for stock and vol)
        dw_ind = np.random.normal(size=[num_sample, 2, self.num_time_interval]) * self.sqrt_delta_t
        
        # Correlate Brownian motions
        dw_sample = np.zeros_like(dw_ind)
        dw_sample[:, 0, :] = dw_ind[:, 0, :]  # dW1 (stock)
        dw_sample[:, 1, :] = (self.rho * dw_ind[:, 0, :] + np.sqrt(1 - self.rho**2) * dw_ind[:, 1, :])
        
        # Initialize state tensor [num_samples, 2 (stock/var), timesteps]
        x_sample = np.zeros([num_sample, 2, self.num_time_interval + 1])
        x_sample[:, :, 0] = np.tile(self.x_init, (num_sample, 1))
        
        # Euler-Maruyama simulation
        for i in range(self.num_time_interval): # Loop over each time-step
            S = x_sample[:, 0, i]  # Extract current stock price at time-step i
            v = x_sample[:, 1, i]  # Extract current variance at time-step i
            
            # Avoid negative variances (absorption)
            sqrt_v = np.sqrt(np.maximum(v, 0))
            
            # Stock price increment - Diffusion term (r*S*delta(t)) + drift term (sqrt(v)*S*dw_1, wheredw_1=correlated Brownian motion for stock price)
            dS = self.r * S * self.delta_t + sqrt_v * S * dw_sample[:, 0, i]
            
            # Variance increment - Diffusion term (kappa*(theta-v)*delta(t)) + drift term (eplison*sqrt(v)*dw_2, for dw_2=col Brownian motion for volatility)
            dv = (self.kappa * (self.theta - v) * self.delta_t + self.epsilon * sqrt_v * dw_sample[:, 1, i]) # 
            
            # Update state with absorption for variance
            x_sample[:, 0, i+1] = S + dS # S= S + dS
            x_sample[:, 1, i+1] = np.maximum(v + dv, 0) # V=V + dV
    ...
    def g_tf(self, t, x):
        ...
        S_T = x[:, 0]  # Terminal stock price
        return tf.expand_dims(tf.nn.relu(S_T - self.strike), 1)
